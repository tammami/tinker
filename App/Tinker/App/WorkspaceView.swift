    ...
    public var body: some View {
        ...
        .sheet(item: boundWorkspace.pendingTool) { request in
            ToolsWizardSheet(
                ...
                onOpenScript: { connectionID, script in newQuery(connectionID, script) },
                onDismiss: {
                    workspace.pendingTool = nil
                    controller.refresh()
                    for config in environment.connections { Task { await sidebar.refresh(connectionID: config.id) } }
                })
        }
