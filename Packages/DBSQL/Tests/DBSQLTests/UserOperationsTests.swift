...
final class UserOperationsTests: XCTestCase {
    func testCreateOnPostgresQuotesEverythingAndGrantsOnPublic() throws {
        ...
        XCTAssertEqual(try UserOperations.create(request, dialect: .postgresql), [
    ...
    func testCreateOnMySQLUsesAccountStrings() throws {
        ...
        XCTAssertEqual(try UserOperations.create(request, dialect: .mysql), [
        ...
        XCTAssertEqual(try UserOperations.create(root, dialect: .mysql).last, "GRANT ALL PRIVILEGES ON *.* TO 'admin'@'localhost' WITH GRANT OPTION")
    }
    ...
    func testAlterAndDrop() throws {
        ...
        XCTAssertEqual(try UserOperations.alter(request, dialect: .mysql), [], "nothing to change without a password or grants")
        XCTAssertEqual(
